...
var G = 9e15, $ = 1e9, ue = "0123456789abcdef", x = "2.", y = "3.", fe = {
  ...
  maxE: G,
  crypto: !1
}, me, U, m = !0, ie = "[DecimalError] ", V = ie + "Invalid argument: ", Ne = ie + "Precision limit exceeded", ve = ie + "crypto unavailable", Ee = "[object Decimal]", A = Math.floor, q = Math.pow, Ae = /^0b([01]+(\.[01]*)?|\.[01]+)(p[+-]?\d+)?$/i, Le = /^0x([0-9a-f]+(\.[0-9a-f]*)?|\.[0-9a-f]+)(p[+-]?\d+)?$/i, Oe = /^0o([0-7]+(\.[0-7]*)?|\.[0-7]+)(p[+-]?\d+)?$/i, Me = /^(\d+(\.\d*)?|\.\d+)(e[+-]?\d+)?$/i, _ = 1e7, w = 7, De = 9007199254740991, Ie = x.length - 1, ce = y.length - 1, d = { toStringTag: Ee };
...
d.cosine = d.cos = function() {
  ...
  return n.d ? n.d[0] ? (e = r.precision, i = r.rounding, r.precision = e + Math.max(n.e, n.sd()) + w, r.rounding = 1, n = be(r, Pe(r, n)), r.precision = e, r.rounding = i, p(U == 2 || U == 3 ? n.neg() : n, e, i, !0)) : new r(1) : new r(NaN);
};
d.cubeRoot = d.cbrt = function() {
  var e, i, n, r, t, s, o, u, c, f, l = this, a = l.constructor;
  if (!l.isFinite() || l.isZero())
    return new a(l);
  for (m = !1, s = l.s * q(l.s * l, 1 / 3), !s || Math.abs(s) == 1 / 0 ? (n = S(l.d), e = l.e, (s = (e - n.length + 1) % 3) && (n += s == 1 || s == -2 ? "0" : "00"), s = q(n, 1 / 3), e = A((e + 1) / 3) - (e % 3 == (e < 0 ? -1 : 2)), s == 1 / 0 ? n = "5e" + e : (n = s.toExponential(), n = n.slice(0, n.indexOf("e") + 1) + e), r = new a(n), r.s = l.s) : r = new a(s.toString()), o = (e = a.precision) + 3; ; )
    if (u = r, c = u.times(u).times(u), f = c.plus(l), r = M(f.plus(l).times(u), f.plus(c), o + 2, 1), S(u.d).slice(0, o) === (n = S(r.d)).slice(0, o))
...
d.decimalPlaces = d.dp = function() {
  ...
  if (i) {
    if (e = i.length - 1, n = (e - A(this.e / w)) * w, e = i[e], e)
      for (; e % 10 == 0; e /= 10)
        n--;
    n < 0 && (n = 0);
  }
...
d.dividedToIntegerBy = d.divToInt = function(e) {
  ...
  return p(M(i, new n(e), 0, 1, 1), n.precision, n.rounding);
};
...
d.hyperbolicTangent = d.tanh = function() {
  var e, i, n = this, r = n.constructor;
  return n.isFinite() ? n.isZero() ? new r(n) : (e = r.precision, i = r.rounding, r.precision = e + 7, r.rounding = 1, M(n.sinh(), n.cosh(), r.precision = e, r.rounding = i)) : new r(n.s);
};
d.inverseCosine = d.acos = function() {
  var e, i = this, n = i.constructor, r = i.abs().cmp(1), t = n.precision, s = n.rounding;
  return r !== -1 ? r === 0 ? i.isNeg() ? T(n, t, s) : new n(0) : new n(NaN) : i.isZero() ? T(n, t + 4, s).times(0.5) : (n.precision = t + 6, n.rounding = 1, i = i.asin(), e = T(n, t + 4, s).times(0.5), n.precision = t, n.rounding = s, e.minus(i));
};
...
d.inverseHyperbolicTangent = d.atanh = function() {
  var e, i, n, r, t = this, s = t.constructor;
  return t.isFinite() ? t.e >= 0 ? new s(t.abs().eq(1) ? t.s / 0 : t.isZero() ? t : NaN) : (e = s.precision, i = s.rounding, r = t.sd(), Math.max(r, e) < 2 * -t.e - 1 ? p(new s(t), e, i, !0) : (s.precision = n = r - t.e, t = M(t.plus(1), new s(1).minus(t), n + e, 1), s.precision = e + 4, s.rounding = 1, t = t.ln(), s.precision = e, s.rounding = i, t.times(0.5))) : new s(NaN);
};
d.inverseSine = d.asin = function() {
  var e, i, n, r, t = this, s = t.constructor;
  return t.isZero() ? new s(t) : (i = t.abs().cmp(1), n = s.precision, r = s.rounding, i !== -1 ? i === 0 ? (e = T(s, n + 4, r).times(0.5), e.s = t.s, e) : new s(NaN) : (s.precision = n + 6, s.rounding = 1, t = t.div(new s(1).minus(t.times(t)).sqrt().plus(1)).atan(), s.precision = n, s.rounding = r, t.times(2)));
};
d.inverseTangent = d.atan = function() {
  ...
  if (f.isFinite()) {
    ...
    if (f.abs().eq(1) && a + 4 <= ce)
      return o = T(l, a + 4, h).times(0.25), o.s = f.s, o;
  } else {
    if (!f.s)
      return new l(NaN);
    if (a + 4 <= ce)
      return o = T(l, a + 4, h).times(0.5), o.s = f.s, o;
  }
...
d.logarithm = d.log = function(e) {
  ...
  if (m = !1, u = a + g, o = H(f, u), r = i ? ee(l, u + 10) : H(e, u), c = M(o, r, u, 1), J(c.d, t = a, h))
    do
      if (u += 10, o = H(f, u), r = i ? ee(l, u + 10) : H(e, u), c = M(o, r, u, 1), !s) {
...
d.minus = d.sub = function(e) {
  ...
  if (n = A(e.e / w), l = A(g.e / w), f = f.slice(), s = l - n, s) {
...
d.modulo = d.mod = function(e) {
  ...
  return e = new r(e), !n.d || !e.s || e.d && !e.d[0] ? new r(NaN) : !e.d || n.d && !n.d[0] ? p(new r(n), r.precision, r.rounding) : (m = !1, r.modulo == 9 ? (i = M(n, e.abs(), 0, 3, 1), i.s *= e.s) : i = M(n, e, 0, r.modulo, 1), i = i.times(e), m = !0, n.minus(i));
};
...
d.plus = d.add = function(e) {
  ...
  if (s = A(a.e / w), r = A(e.e / w), f = f.slice(), t = s - r, t) {
...
d.sine = d.sin = function() {
  var e, i, n = this, r = n.constructor;
  return n.isFinite() ? n.isZero() ? new r(n) : (e = r.precision, i = r.rounding, r.precision = e + Math.max(n.e, n.sd()) + w, r.rounding = 1, n = _e(r, Pe(r, n)), r.precision = e, r.rounding = i, p(U > 2 ? n.neg() : n, e, i, !0)) : new r(NaN);
};
d.squareRoot = d.sqrt = function() {
  var e, i, n, r, t, s, o = this, u = o.d, c = o.e, f = o.s, l = o.constructor;
  if (f !== 1 || !u || !u[0])
    return new l(!f || f < 0 && (!u || u[0]) ? NaN : u ? o : 1 / 0);
  for (m = !1, f = Math.sqrt(+o), f == 0 || f == 1 / 0 ? (i = S(u), (i.length + c) % 2 == 0 && (i += "0"), f = Math.sqrt(i), c = A((c + 1) / 2) - (c < 0 || c % 2), f == 1 / 0 ? i = "5e" + c : (i = f.toExponential(), i = i.slice(0, i.indexOf("e") + 1) + c), r = new l(i)) : r = new l(f.toString()), n = (c = l.precision) + 3; ; )
    if (s = r, r = s.plus(M(o, s, n + 2, 1)).times(0.5), S(s.d).slice(0, n) === (i = S(r.d)).slice(0, n))
...
d.tangent = d.tan = function() {
  var e, i, n = this, r = n.constructor;
  return n.isFinite() ? n.isZero() ? new r(n) : (e = r.precision, i = r.rounding, r.precision = e + 10, r.rounding = 1, n = n.sin(), n.s = 1, n = M(n, new r(1).minus(n.times(n)).sqrt(), e + 10, 0), r.precision = e, r.rounding = i, p(U == 2 || U == 4 ? n.neg() : n, e, i, !0)) : new r(NaN);
};
d.times = d.mul = function(e) {
  ...
  for (n = A(l.e / w) + A(e.e / w), c = h.length, f = g.length, c < f && (s = h, h = g, g = s, o = c, c = f, f = o), s = [], o = c + f, r = o; r--; )
    s.push(0);
  for (r = f; --r >= 0; ) {
    for (i = 0, t = c + r; t > r; )
      u = s[t] + g[r] * h[t - r - 1] + i, s[t--] = u % _ | 0, i = u / _ | 0;
    s[t] = (s[t] + i) % _ | 0;
  }
  for (; !s[--o]; )
    s.pop();
...
d.toExponential = function(e, i) {
  var n, r = this, t = r.constructor;
  return e === void 0 ? n = R(r, !0) : (O(e, 0, $), i === void 0 ? i = t.rounding : O(i, 0, 8), r = p(new t(r), e + 1, i), n = R(r, !0, e + 1)), r.isNeg() && !r.isZero() ? "-" + n : n;
};
d.toFixed = function(e, i) {
  var n, r, t = this, s = t.constructor;
  return e === void 0 ? n = R(t) : (O(e, 0, $), i === void 0 ? i = s.rounding : O(i, 0, 8), r = p(new s(t), e + t.e + 1, i), n = R(r, !1, e + r.e + 1)), t.isNeg() && !t.isZero() ? "-" + n : n;
};
d.toFraction = function(e) {
  ...
  for (m = !1, u = new N(S(v)), l = N.precision, N.precision = s = v.length * w * 2; a = M(u, i, 0, 1, 1), t = n.plus(a.times(r)), t.cmp(e) != 1; )
    n = r, r = t, t = f, f = c.plus(a.times(t)), c = t, t = i, i = u.minus(a.times(t)), u = t;
  return t = M(e.minus(n), r, 0, 1, 1), c = c.plus(t.times(f)), n = n.plus(t.times(r)), c.s = f.s = g.s, h = M(f, r, s, 1).minus(g).abs().cmp(M(c, n, s, 1).minus(g).abs()) < 1 ? [f, r] : [c, n], N.precision = l, m = !0, h;
};
...
d.toNearest = function(e, i) {
  ...
  return e.d[0] ? (m = !1, n = M(n, e, 0, i, 1).times(e), m = !0, p(n)) : (e.s = n.s, n = e), n;
};
d.toNumber = function() {
  return +this;
};
...
d.toPower = d.pow = function(e) {
  ...
  if (r = c.precision, s = c.rounding, e.eq(1))
    return p(u, r, s);
  if (i = A(e.e / w), i >= e.d.length - 1 && (n = f < 0 ? -f : f) <= De)
    return t = Ce(c, u, n, r), e.s < 0 ? new c(1).div(t) : p(t, r, s);
  if (o = u.s, o < 0) {
    if (i < e.d.length - 1)
      return new c(NaN);
...
var M = function() {
  ...
  return function(r, t, s, o, u, c) {
    var f, l, a, h, g, v, N, L, C, I, E, P, K, Z, te, Q, X, se, b, Y, j = r.constructor, oe = r.s == t.s ? 1 : -1, F = r.d, k = t.d;
    if (!F || !F[0] || !k || !k[0])
      return new j(
        !r.s || !t.s || (F ? k && F[0] == k[0] : !k) ? NaN : F && F[0] == 0 || !k ? oe * 0 : oe / 0
      );
    for (c ? (g = 1, l = r.e - t.e) : (c = _, g = w, l = A(r.e / g) - A(t.e / g)), b = k.length, X = F.length, C = new j(oe), I = C.d = [], a = 0; k[a] == (F[a] || 0); a++)
      ;
    if (k[a] > (F[a] || 0) && l--, s == null ? (Z = s = j.precision, o = j.rounding) : u ? Z = s + (r.e - t.e) + 1 : Z = s, Z < 0)
      I.push(1), v = !0;
    else {
      if (Z = Z / g + 2 | 0, a = 0, b == 1) {
        for (h = 0, k = k[0], Z++; (a < X || h) && Z--; a++)
          te = h * c + (F[a] || 0), I[a] = te / k | 0, h = te % k | 0;
        v = h || a < X;
      } else {
        for (h = c / (k[0] + 1) | 0, h > 1 && (k = e(k, h, c), F = e(F, h, c), b = k.length, X = F.length), Q = b, E = F.slice(0, b), P = E.length; P < b; )
          E[P++] = 0;
        Y = k.slice(), Y.unshift(0), se = k[0], k[1] >= c / 2 && ++se;
        do
          h = 0, f = i(k, E, b, P), f < 0 ? (K = E[0], b != P && (K = K * c + (E[1] || 0)), h = K / se | 0, h > 1 ? (h >= c && (h = c - 1), N = e(k, h, c), L = N.length, P = E.length, f = i(N, E, L, P), f == 1 && (h--, n(N, b < L ? Y : k, L, c))) : (h == 0 && (f = h = 1), N = k.slice()), L = N.length, L < P && N.unshift(0), n(E, N, P, c), f == -1 && (P = E.length, f = i(k, E, b, P), f < 1 && (h++, n(E, b < P ? Y : k, P, c))), P = E.length) : f === 0 && (h++, E = [0]), I[a++] = h, f && E[0] ? E[P++] = F[Q] || 0 : (E = [F[Q]], P = 1);
        while ((Q++ < X || E[0] !== void 0) && Z--);
        v = E[0] !== void 0;
      }
      I[0] || I.shift();
    }
...
function ee(e, i, n) {
  if (i > Ie)
    throw m = !0, n && (e.precision = n), Error(Ne);
...
function Ce(e, i, n, r) {
  ...
  for (m = !1; ; ) {
    if (n % 2 && (s = s.times(i), pe(s.d, o) && (t = !0)), n = A(n / 2), n === 0) {
...
function le(e, i) {
  ...
  for (r = Math.log(q(2, a)) / Math.LN10 * 2 + 5 | 0, c += r, n = s = o = new h(1), h.precision = c; ; ) {
    if (s = p(s.times(e), c, 1), n = n.times(++l), u = o.plus(M(s, n, c, 1)), S(u.d).slice(0, c) === S(o.d).slice(0, c)) {
      for (t = a; t--; )
        o = p(o.times(o), c, 1);
      if (i == null)
...
function H(e, i) {
  var n, r, t, s, o, u, c, f, l, a, h, g = 1, v = 10, N = e, L = N.d, C = N.constructor, I = C.rounding, E = C.precision;
  if (N.s < 0 || !L || !L[0] || !N.e && L[0] == 1 && L.length == 1)
    return new C(L && !L[0] ? -1 / 0 : N.s != 1 ? NaN : L ? 0 : N);
  if (i == null ? (m = !1, l = E) : l = i, C.precision = l += v, n = S(L), r = n.charAt(0), Math.abs(s = N.e) < 15e14) {
    for (; r < 7 && r != 1 || r == 1 && n.charAt(1) > 3; )
      N = N.times(e), n = S(N.d), r = n.charAt(0), g++;
    s = N.e, r > 1 ? (N = new C("0." + n), s++) : N = new C(r + "." + n.slice(1));
  } else
    return f = ee(C, l + 2, E).times(s + ""), N = H(new C(r + "." + n.slice(1)), l - v).plus(f), C.precision = E, i == null ? p(N, E, I, m = !0) : N;
  for (a = N, c = o = N = M(N.minus(1), N.plus(1), l, 1), h = p(N.times(N), l, 1), t = 3; ; ) {
    if (o = p(o.times(h), l, 1), f = c.plus(M(o, new C(t), l, 1)), S(f.d).slice(0, l) === S(c.d).slice(0, l))
      if (c = c.times(2), s !== 0 && (c = c.plus(ee(C, l + 2, E).times(s + ""))), c = M(c, new C(g), l, 1), i == null)
        if (J(c.d, l - v, I, u))
          C.precision = l += v, f = o = N = M(a.minus(1), a.plus(1), l, 1), h = p(N.times(N), l, 1), t = u = 1;
        else
          return p(c, C.precision = E, I, m = !0);
      else
...
function Te(e, i) {
  ...
  if (i.indexOf("_") > -1) {
    if (i = i.replace(/(\d)_(?=\d)/g, "$1"), Me.test(i))
      return ae(e, i);
  ...
  if (Le.test(i))
    ...
    throw Error(V + i);
  for (s = i.search(/p/i), s > 0 ? (c = +i.slice(s + 1), i = i.substring(2, s)) : i = i.slice(2), s = i.indexOf("."), o = s >= 0, r = e.constructor, o && (i = i.replace(".", ""), u = i.length, s = u - s, t = Ce(r, new r(n), s, s * 2)), f = z(i, n, _), l = f.length - 1, s = l; f[s] === 0; --s)
    f.pop();
  return s < 0 ? new r(e.s * 0) : (e.e = ne(f, l), e.d = f, m = !1, o && (e = M(e, t, u * 4)), c && (e = e.times(Math.abs(c) < 54 ? q(2, c) : D.pow(2, c))), m = !0, e);
}
function _e(e, i) {
  var n, r = i.d.length;
  if (r < 3)
...
function W(e, i, n, r, t) {
  ...
  for (m = !1, c = n.times(n), u = new e(r); ; ) {
    if (o = M(u.times(c), new e(i++ * i++), f, 1), u = t ? r.plus(o) : r.minus(o), r = M(o.times(c), new e(i++ * i++), f, 1), o = u.plus(r), o.d[l] !== void 0) {
...
function Pe(e, i) {
  var n, r = i.s < 0, t = T(e, e.precision, 1), s = t.times(0.5);
  if (i = i.abs(), i.lte(s))
    return U = r ? 4 : 1, i;
...
function he(e, i, n, r) {
  ...
  if (v ? (O(n, 1, $), r === void 0 ? r = g.rounding : O(r, 0, 8)) : (n = g.precision, r = g.rounding), !e.isFinite())
    ...
    for (l = R(e), o = l.indexOf("."), v ? (t = 2, i == 16 ? n = n * 4 - 3 : i == 8 && (n = n * 3 - 2)) : t = i, o >= 0 && (l = l.replace(".", ""), h = new g(1), h.e = l.length - o, h.d = z(R(h), 10, t), h.e = h.d.length), a = z(l, 10, t), s = c = a.length; a[--c] == 0; )
      a.pop();
    if (!a[0])
      l = v ? "0p+0" : "0";
    else {
      if (o < 0 ? s-- : (e = new g(e), e.d = a, e.e = s, e = M(e, h, n, r, 0, t), a = e.d, s = e.e, f = me), o = a[n], u = t / 2, f = f || a[n + 1] !== void 0, f = r < 4 ? (o !== void 0 || f) && (r === 0 || r === (e.s < 0 ? 3 : 2)) : o > u || o === u && (r === 4 || f || r === 6 && a[n - 1] & 1 || r === (e.s < 0 ? 8 : 7)), a.length = n, f)
        for (; ++a[--n] > t - 1; )
...
function We(e, i) {
  ...
  return !e.s || !i.s ? n = new this(NaN) : !e.d && !i.d ? (n = T(this, s, 1).times(i.s > 0 ? 0.25 : 0.75), n.s = e.s) : !i.d || e.isZero() ? (n = i.s < 0 ? T(this, r, t) : new this(0), n.s = e.s) : !e.d || i.isZero() ? (n = T(this, s, 1).times(0.5), n.s = e.s) : i.s < 0 ? (this.precision = s, this.rounding = 1, n = this.atan(M(e, i, s, 1)), i = T(this, s, 1), this.precision = r, this.rounding = t, n = e.s < 0 ? n.minus(i) : n.plus(i)) : n = this.atan(M(e, i, s, 1)), n;
}
function Xe(e) {
...
function Fe(e) {
  ...
  function t(s) {
    ...
    if (c = typeof s, c === "number") {
      ...
      throw Error(V + s);
    return (u = s.charCodeAt(0)) === 45 ? (s = s.slice(1), f.s = -1) : (u === 43 && (s = s.slice(1)), f.s = 1), Me.test(s) ? ae(f, s) : Te(f, s);
  }
  if (t.prototype = d, t.ROUND_UP = 0, t.ROUND_DOWN = 1, t.ROUND_CEIL = 2, t.ROUND_FLOOR = 3, t.ROUND_HALF_UP = 4, t.ROUND_HALF_DOWN = 5, t.ROUND_HALF_EVEN = 6, t.ROUND_HALF_CEIL = 7, t.ROUND_HALF_FLOOR = 8, t.EUCLID = 9, t.config = t.set = Qe, t.clone = Fe, t.isDecimal = ge, t.abs = Re, t.acos = Ze, t.acosh = Ue, t.add = Be, t.asin = He, t.asinh = Ve, t.atan = $e, t.atanh = Ge, t.atan2 = We, t.cbrt = Xe, t.ceil = Je, t.clamp = Ke, t.cos = Ye, t.cosh = je, t.div = ze, t.exp = xe, t.floor = ye, t.hypot = ei, t.ln = ii, t.log = ni, t.log10 = ti, t.log2 = ri, t.max = si, t.min = oi, t.mod = ui, t.mul = fi, t.pow = ci, t.random = li, t.round = ai, t.sign = hi, t.sin = di, t.sinh = pi, t.sqrt = gi, t.sub = wi, t.sum = mi, t.tan = Ni, t.tanh = vi, t.trunc = Ei, e === void 0 && (e = {}), e && e.defaults !== !0)
...
function ge(e) {
  return e instanceof D || e && e.toStringTag === Ee || !1;
}
function ii(e) {
...
function si() {
  return qe(this, arguments, "lt");
}
function oi() {
  return qe(this, arguments, "gt");
}
function ui(e, i) {
...
var D = d.constructor = Fe(fe);
x = new D(x);
y = new D(y);
const we = {
  add: function(e, i) {
    return new D(e).add(new D(i)).toNumber();
  },
  subtract: function(e, i) {
    return new D(e).sub(new D(i)).toNumber();
  },
  multiply: function(e, i) {
    return new D(e).mul(new D(i)).toNumber();
  },
  divide: function(e, i) {
    return new D(e).div(new D(i)).toNumber();
  }
};
Number.prototype.toFixedNew = function(e) {
  let i = this;
  return i < 0 ? (i = i * -1, (Math.round(we.multiply(i, Math.pow(10, e))) / Math.pow(10, e) * -1).toFixed(e)) : (Math.round(we.multiply(i, Math.pow(10, e))) / Math.pow(10, e)).toFixed(e);
};
